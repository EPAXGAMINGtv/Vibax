...
public class CopyDirectoryHandler extends AuthenticatedHandler implements HttpHandler {

    public CopyDirectoryHandler(String passwordHash) {
        super(passwordHash);
    }
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (source == null || target == null) {
            sendText(exchange, 400, "Required parameters: source, target");
            return;
        }

        try {
            FileManager.copyDirectory(source, target);
            sendText(exchange, 200, "Directory copied: " + source + " -> " + target);
        } catch (Exception e) {
            sendText(exchange, 500, "Directory copy failed: " + e.getMessage());
        }
    }
